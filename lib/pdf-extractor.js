// ApplyPilot AI - Zero-Dependency PDF & Text File Extractor
// Extracts pure text streams directly in browser without heavy external packages.
// Handles: Deflate (RFC 1950), Deflate-Raw (RFC 1951), Hex strings <...>, TJ arrays, UTF-16BE.

async function safeDecompress(bytes) {
  if (typeof DecompressionStream === 'undefined') return bytes;
  for (const format of ['deflate', 'deflate-raw']) {
    try {
      const ds = new DecompressionStream(format);
      const stream = new Blob([bytes]).stream().pipeThrough(ds);
      const res = await new Response(stream).arrayBuffer();
      return new Uint8Array(res);
    } catch (e) {
      // Try next format
    }
  }
  return bytes;
}

// PNG predictors (RFC 2083) applied by FlateDecode with /Predictor >= 10.
// Without undoing these, decompressed content-stream bytes are shifted garbage,
// which is the single most common reason text extraction silently returns junk.
function undoPngPredictor(data, colors, bitsPerComponent, columns) {
  const bpp = Math.max(1, Math.ceil((colors * bitsPerComponent) / 8));
  const rowLength = Math.ceil((colors * bitsPerComponent * columns) / 8);
  const rowStride = rowLength + 1; // leading filter-type byte
  if (rowStride <= 1 || data.length < rowStride) return data;

  const rows = Math.floor(data.length / rowStride);
  const out = new Uint8Array(rows * rowLength);
  let prevRow = new Uint8Array(rowLength);

  for (let r = 0; r < rows; r++) {
    const filter = data[r * rowStride];
    const src = data.subarray(r * rowStride + 1, r * rowStride + 1 + rowLength);
    const row = new Uint8Array(rowLength);

    for (let i = 0; i < rowLength; i++) {
      const raw = src[i] || 0;
      const left = i >= bpp ? row[i - bpp] : 0;
      const up = prevRow[i];
      const upLeft = i >= bpp ? prevRow[i - bpp] : 0;

      let value;
      switch (filter) {
        case 0: value = raw; break;                       // None
        case 1: value = raw + left; break;                // Sub
        case 2: value = raw + up; break;                  // Up
        case 3: value = raw + ((left + up) >> 1); break;  // Average
        case 4: {                                         // Paeth
          const p = left + up - upLeft;
          const pa = Math.abs(p - left);
          const pb = Math.abs(p - up);
          const pc = Math.abs(p - upLeft);
          value = raw + ((pa <= pb && pa <= pc) ? left : (pb <= pc ? up : upLeft));
          break;
        }
        default: value = raw; break;
      }
      row[i] = value & 0xff;
    }

    out.set(row, r * rowLength);
    prevRow = row;
  }

  return out;
}

// Parse the /DecodeParms of a stream dictionary and reverse any predictor.
function applyDecodeParms(bytes, dict) {
  if (!dict) return bytes;
  const predictorMatch = dict.match(/\/Predictor\s+(\d+)/);
  if (!predictorMatch) return bytes;

  const predictor = parseInt(predictorMatch[1], 10);
  if (!(predictor >= 10)) return bytes; // TIFF predictor 2 is not used for text

  const colors = parseInt((dict.match(/\/Colors\s+(\d+)/) || [, '1'])[1], 10);
  const bpc = parseInt((dict.match(/\/BitsPerComponent\s+(\d+)/) || [, '8'])[1], 10);
  const columns = parseInt((dict.match(/\/Columns\s+(\d+)/) || [, '1'])[1], 10);

  try {
    return undoPngPredictor(bytes, colors, bpc, columns);
  } catch (e) {
    return bytes;
  }
}

// Build a CID -> Unicode map from a /ToUnicode CMap so subset-embedded fonts
// (what Word, Canva and LaTeX emit) decode to real characters instead of the
// arbitrary glyph indices stored in the content stream.
function parseToUnicodeCMap(cmapText) {
  const map = new Map();
  if (!cmapText) return map;

  const hexToStr = (hex) => {
    let out = '';
    for (let i = 0; i + 3 < hex.length + 1; i += 4) {
      const code = parseInt(hex.substring(i, i + 4), 16);
      if (!isNaN(code) && code > 0) out += String.fromCharCode(code);
    }
    return out;
  };

  // bfchar: <src> <dst>
  const charSections = cmapText.match(/beginbfchar([\s\S]*?)endbfchar/g) || [];
  for (const section of charSections) {
    const pairRe = /<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>/g;
    let m;
    while ((m = pairRe.exec(section)) !== null) {
      map.set(parseInt(m[1], 16), hexToStr(m[2]));
    }
  }

  // bfrange: <lo> <hi> <dstStart>  (the array form is rare in resumes)
  const rangeSections = cmapText.match(/beginbfrange([\s\S]*?)endbfrange/g) || [];
  for (const section of rangeSections) {
    const rangeRe = /<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>\s*<([0-9a-fA-F]+)>/g;
    let m;
    while ((m = rangeRe.exec(section)) !== null) {
      const lo = parseInt(m[1], 16);
      const hi = parseInt(m[2], 16);
      const dst = parseInt(m[3], 16);
      if (hi - lo > 65535) continue; // guard against a malformed range
      for (let c = lo; c <= hi; c++) {
        map.set(c, String.fromCharCode(dst + (c - lo)));
      }
    }
  }

  return map;
}

function decodePdfHexString(hex) {
  const cleanHex = hex.replace(/\s+/g, '');
  if (cleanHex.length % 2 !== 0) return '';
  
  // Check if UTF-16BE (starts with FEFF or contains 00xx patterns)
  const isUtf16 = cleanHex.startsWith('feff') || cleanHex.startsWith('FEFF') || 
                  (cleanHex.length >= 4 && cleanHex.substring(0, 2) === '00');

  let str = '';
  if (isUtf16) {
    const start = (cleanHex.startsWith('feff') || cleanHex.startsWith('FEFF')) ? 4 : 0;
    for (let i = start; i < cleanHex.length; i += 4) {
      const code = parseInt(cleanHex.substring(i, i + 4), 16);
      if (!isNaN(code) && code > 0) str += String.fromCharCode(code);
    }
  } else {
    for (let i = 0; i < cleanHex.length; i += 2) {
      const code = parseInt(cleanHex.substring(i, i + 2), 16);
      if (!isNaN(code) && code >= 32 && code <= 126) str += String.fromCharCode(code);
      else if (code === 10 || code === 13 || code === 9) str += ' ';
    }
  }
  return str;
}

// Decode a PDF hex string, preferring the document's ToUnicode map when it
// covers the codes. Falls back to the plain interpretation for simple fonts.
function decodeHexWithCMap(hex, cmap) {
  if (!cmap || cmap.size === 0) return decodePdfHexString(hex);

  const clean = hex.replace(/\s+/g, '');
  if (clean.length % 4 !== 0) return decodePdfHexString(hex);

  let out = '';
  let hits = 0;
  let total = 0;
  for (let i = 0; i < clean.length; i += 4) {
    const code = parseInt(clean.substring(i, i + 4), 16);
    if (isNaN(code)) continue;
    total++;
    if (cmap.has(code)) {
      out += cmap.get(code);
      hits++;
    }
  }

  // Only trust the CMap result if it explained most of the string.
  if (total > 0 && hits / total >= 0.5) return out;
  return decodePdfHexString(hex);
}

function cleanPdfText(text) {
  return text
    .replace(/\\(\d{3})/g, (_, oct) => String.fromCharCode(parseInt(oct, 8)))
    .replace(/\\([()\\])/g, '$1')
    .replace(/\\r/g, ' ')
    .replace(/\\n/g, ' ')
    .replace(/\\t/g, ' ')
    // Keep printable Unicode (accented names, non-Latin scripts) and strip only
    // control characters. The previous ASCII-only filter mangled real resumes.
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F\uFFFD]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

const PdfExtractor = {
  // Read File object (PDF, DOCX, TXT) and extract text
  async extractText(file) {
    if (!file) return "";

    const name = (file.name || "").toLowerCase();

    // 1. Plain text files
    if (name.endsWith('.txt') || file.type === 'text/plain') {
      return await file.text();
    }

    // 2. PDF extraction
    if (name.endsWith('.pdf') || file.type === 'application/pdf') {
      return await this.extractFromPdf(file);
    }

    // 3. Fallback: attempt to read as text and filter printable ASCII
    const raw = await file.text();
    return cleanPdfText(raw);
  },

  // Extract text from PDF using stream scanning, decompression, and operator parsing
  async extractFromPdf(file) {
    const arrayBuffer = await file.arrayBuffer();
    const bytes = new Uint8Array(arrayBuffer);
    
    // Convert to latin1 binary string for stream searching
    let binary = "";
    const chunkSize = 8192;
    for (let i = 0; i < bytes.length; i += chunkSize) {
      binary += String.fromCharCode.apply(null, bytes.subarray(i, i + chunkSize));
    }

    const textPieces = [];

    // Pass 1: collect every /ToUnicode CMap in the file so glyph ids from
    // subset-embedded fonts can be mapped back to real characters.
    const toUnicode = new Map();
    const cmapStreamRegex = /\/ToUnicode[\s\S]{0,200}?stream\r?\n([\s\S]*?)\r?\nendstream/g;
    let cmapMatch;
    while ((cmapMatch = cmapStreamRegex.exec(binary)) !== null) {
      const raw = cmapMatch[1];
      const rawBytes = new Uint8Array(raw.length);
      for (let j = 0; j < raw.length; j++) rawBytes[j] = raw.charCodeAt(j);
      const inflated = await safeDecompress(rawBytes);
      let text = "";
      for (let k = 0; k < inflated.length; k += chunkSize) {
        text += String.fromCharCode.apply(null, inflated.subarray(k, k + chunkSize));
      }
      for (const [code, ch] of parseToUnicodeCMap(text)) {
        if (!toUnicode.has(code)) toUnicode.set(code, ch);
      }
    }

    // Find all 'stream ... endstream' blocks, keeping the preceding dictionary so
    // /DecodeParms predictors can be reversed.
    const streamRegex = /(<<[\s\S]{0,600}?>>)?\s*stream\r?\n([\s\S]*?)\r?\nendstream/g;
    let match;

    while ((match = streamRegex.exec(binary)) !== null) {
      const dict = match[1];
      const rawStream = match[2];
      const streamBytes = new Uint8Array(rawStream.length);
      for (let j = 0; j < rawStream.length; j++) {
        streamBytes[j] = rawStream.charCodeAt(j);
      }

      // Decompress with fallback across deflate and deflate-raw, then undo any
      // PNG predictor declared in the stream dictionary.
      const inflated = await safeDecompress(streamBytes);
      const decompressedBytes = applyDecodeParms(inflated, dict);
      let decoded = "";
      for (let k = 0; k < decompressedBytes.length; k += chunkSize) {
        decoded += String.fromCharCode.apply(null, decompressedBytes.subarray(k, k + chunkSize));
      }

      // Extract text within BT ... ET operators
      const btRegex = /BT[\s\S]*?ET/g;
      let btMatch;
      while ((btMatch = btRegex.exec(decoded)) !== null) {
        const block = btMatch[0];

        // 1. Literal strings: (text) Tj, (text) ', (text) "
        const tjRegex = /\(([^)]+)\)\s*(?:Tj|'|")/g;
        let tjMatch;
        while ((tjMatch = tjRegex.exec(block)) !== null) {
          if (tjMatch[1]) textPieces.push(tjMatch[1]);
        }

        // 2. Hex strings: <48656c6c6f> Tj
        const hexTjRegex = /<([0-9a-fA-F]+)>\s*(?:Tj|'|")/g;
        let hexTjMatch;
        while ((hexTjMatch = hexTjRegex.exec(block)) !== null) {
          const decodedHex = decodeHexWithCMap(hexTjMatch[1], toUnicode);
          if (decodedHex) textPieces.push(decodedHex);
        }

        // 3. Array of strings: [(part1) 12 <48656c6c6f> (part2)] TJ
        const arrayRegex = /\[(.*?)\]\s*TJ/g;
        let arrayMatch;
        while ((arrayMatch = arrayRegex.exec(block)) !== null) {
          const content = arrayMatch[1];
          const parts = content.match(/\(([^)]+)\)|<([0-9a-fA-F]+)>/g) || [];
          for (const p of parts) {
            if (p.startsWith('(')) {
              textPieces.push(p.slice(1, -1));
            } else if (p.startsWith('<')) {
              const decodedHex = decodeHexWithCMap(p.slice(1, -1), toUnicode);
              if (decodedHex) textPieces.push(decodedHex);
            }
          }
        }
      }
    }

    // Fallback 1: direct literal strings with Tj outside standard BT/ET
    if (textPieces.length === 0) {
      const directTj = /\(([^)]{2,})\)\s*(?:Tj|'|")/g;
      let m;
      while ((m = directTj.exec(binary)) !== null) {
        textPieces.push(m[1]);
      }
    }

    // Fallback 2: search for runs of printable ASCII characters in binary
    if (textPieces.length === 0) {
      const matches = binary.match(/[\x20-\x7E\s]{5,}/g) || [];
      const filtered = matches.filter(s => 
        !s.startsWith('/Root') && !s.startsWith('/Info') && !s.startsWith('/Catalog') &&
        !s.includes('endobj') && !s.includes('xref') && !s.includes('stream') &&
        !s.startsWith('<<') && !s.endsWith('>>')
      );
      if (filtered.length > 0) {
        textPieces.push(filtered.join(' '));
      }
    }

    return cleanPdfText(textPieces.join(' '));
  }
};

// Export for browser and node environments
if (typeof window !== 'undefined') {
  window.PdfExtractor = PdfExtractor;
}
if (typeof self !== 'undefined') {
  self.PdfExtractor = PdfExtractor;
}
if (typeof module !== 'undefined') {
  module.exports = {
    PdfExtractor,
    safeDecompress,
    decodePdfHexString,
    decodeHexWithCMap,
    parseToUnicodeCMap,
    undoPngPredictor,
    applyDecodeParms,
    cleanPdfText
  };
}

